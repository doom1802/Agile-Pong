...
test("submits an anonymous ranked singles result from the QR route", async ({ page }) => {
  ...
  await expect(page.locator("label.field", { hasText: "Points" }).locator("select")).toHaveValue("21")
  ...
  await fillSets(page, ["21", "17", "21", "18"])
  ...
  await expect(quickMatch).toContainText("21-17, 21-18")
...
test("locks a signed-in user into Side A and supports doubles", async ({ page }) => {
  ...
  await fillSets(page, ["21", "17", "18", "21", "21", "19"])
  await page.getByRole("button", { name: "Submit ranked result" }).click()
  ...
  await expect(quickMatch).toContainText("21-17, 18-21, 21-19")
})
